...
AUDIO_PLAYER_CSS = """.tsb-audio-player{position:fixed;right:18px;bottom:18px;z-index:2147483000;display:flex;align-items:center;gap:10px;padding:8px 12px 8px 8px;border:1px solid rgba(255,255,255,.36);border-radius:999px;background:rgba(245,245,247,.76);box-shadow:0 18px 48px rgba(0,0,0,.18);backdrop-filter:saturate(180%) blur(22px)}.tsb-audio-player button{appearance:none;border:0;background:transparent;color:#1d1d1f;cursor:pointer}.tsb-audio-main{width:52px;height:52px;display:grid;place-items:center;padding:0;border-radius:50%;background:#0071e3;color:#fff;box-shadow:0 10px 24px rgba(0,113,227,.28);transition:transform .18s ease,box-shadow .18s ease,filter .18s ease}.tsb-audio-main:hover{transform:translateY(-1px);box-shadow:0 16px 34px rgba(0,113,227,.34)}.tsb-audio-main svg{width:25px;height:25px;fill:currentColor;margin-left:2px}.tsb-audio-player.is-playing .tsb-audio-main{background:#147ce5}.tsb-audio-player.is-blocked .tsb-audio-main{filter:saturate(1.18);box-shadow:0 0 0 3px rgba(0,113,227,.22),0 18px 42px rgba(0,0,0,.2)}.tsb-audio-volume{display:flex;align-items:center;gap:8px;color:#1d1d1f}.tsb-audio-volume-icon{width:24px;height:24px;display:grid;place-items:center;color:#1d1d1f}.tsb-audio-volume-icon svg{width:22px;height:22px;fill:currentColor}.tsb-audio-volume-icon svg path+path{fill:none;stroke:currentColor;stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round}.tsb-audio-volume-range{--volume-fill:100%;appearance:none;width:104px;height:8px;border-radius:999px;background:transparent;outline:none;cursor:pointer}.tsb-audio-volume-range::-webkit-slider-runnable-track{height:8px;border-radius:999px;background:linear-gradient(90deg,#0071e3 var(--volume-fill),rgba(29,29,31,.18) var(--volume-fill))}.tsb-audio-volume-range::-webkit-slider-thumb{appearance:none;width:18px;height:18px;margin-top:-5px;border-radius:50%;background:#fff;border:1px solid rgba(29,29,31,.14);box-shadow:0 4px 14px rgba(0,0,0,.2)}.tsb-audio-volume-range::-moz-range-track{height:8px;border-radius:999px;background:linear-gradient(90deg,#0071e3 var(--volume-fill),rgba(29,29,31,.18) var(--volume-fill))}.tsb-audio-volume-range::-moz-range-thumb{width:18px;height:18px;border:1px solid rgba(29,29,31,.14);border-radius:50%;background:#fff;box-shadow:0 4px 14px rgba(0,0,0,.2)}@media (max-width:720px){.tsb-audio-player{right:14px;bottom:14px;padding:7px 10px 7px 7px}.tsb-audio-main{width:50px;height:50px}.tsb-audio-volume-range{width:88px}}"""
SITE_JS = """(()=>{const TRACKS=__TSB_AUDIO_PLAYLIST__;const script=document.currentScript||[...document.scripts].find(item=>item.src&&item.src.includes("/assets/js/site.js"));const siteRoot=new URL("../../",script&&script.src?script.src:document.baseURI);const stateKey="tsbAudioStateV2";const volumeKey="tsbAudioVolumeV2";const playIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5.4v13.2L18.8 12z"/></svg>';const volumeIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9v6h4l5 4V5L8 9H4z"></path><path d="M16 8.5c1 .9 1.5 2.1 1.5 3.5S17 14.6 16 15.5"></path><path d="M18.6 6c1.6 1.6 2.4 3.6 2.4 6s-.8 4.4-2.4 6"></path></svg>';let player;let audio;let volumeRange;let currentIndex=0;let currentSource=null;let userPaused=false;let blocked=false;let saveTimer=0;function clamp(value,min,max){return Math.min(max,Math.max(min,value));}function readState(){try{return JSON.parse(sessionStorage.getItem(stateKey)||"{}")||{};}catch{return {};}}function saveState(){if(!audio||!currentSource)return;const state={index:currentIndex,src:currentSource.src,time:audio.currentTime||0,volume:audio.volume,userPaused};sessionStorage.setItem(stateKey,JSON.stringify(state));localStorage.setItem(volumeKey,String(audio.volume));}function scheduleSave(){clearTimeout(saveTimer);saveTimer=setTimeout(saveState,250);}function updateVolumeUI(){if(!volumeRange||!audio)return;const value=Math.round(audio.volume*100);volumeRange.value=String(value);volumeRange.style.setProperty("--volume-fill",value+"%");}function pickSource(track){if(!track||!track.sources||!track.sources.length)return null;return track.sources.find(source=>!source.type||audio.canPlayType(source.type))||track.sources[0];}function setPlayerState(){if(!player||!audio)return;player.classList.toggle("is-playing",!audio.paused&&!audio.ended);player.classList.toggle("is-blocked",blocked&&audio.paused&&!userPaused);updateVolumeUI();}function requestPlay(){if(!audio||!currentSource)return;blocked=false;const promise=audio.play();if(promise&&typeof promise.then==="function"){promise.then(()=>{blocked=false;setPlayerState();saveState();}).catch(()=>{blocked=true;setPlayerState();});}else{setPlayerState();}return promise;}function loadTrack(index,options={}){if(!TRACKS.length||!audio)return;currentIndex=(index+TRACKS.length)%TRACKS.length;currentSource=pickSource(TRACKS[currentIndex]);if(!currentSource)return;const nextUrl=new URL(currentSource.src,siteRoot).href;if(audio.src!==nextUrl){audio.src=nextUrl;audio.load();}const resumeTime=Number(options.time||0);if(resumeTime>0){const applyTime=()=>{try{if(Number.isFinite(audio.duration)&&resumeTime<audio.duration-1)audio.currentTime=resumeTime;}catch{}};if(audio.readyState>=1)applyTime();else audio.addEventListener("loadedmetadata",applyTime,{once:true});}if(!userPaused)requestPlay();setPlayerState();}function nextTrack(){loadTrack(currentIndex+1,{time:0});}function setVolume(value){if(!audio)return;audio.volume=clamp(Number(value)/100,0,1);updateVolumeUI();scheduleSave();setPlayerState();}function buildPlayer(){if(!TRACKS.length||document.querySelector("[data-tsb-audio-player]"))return;const state=readState();currentIndex=Number.isInteger(state.index)&&state.index>=0&&state.index<TRACKS.length?state.index:0;userPaused=state.userPaused===true;audio=document.createElement("audio");audio.preload="auto";audio.volume=1;player=document.createElement("div");player.className="tsb-audio-player";player.dataset.tsbAudioPlayer="";const main=document.createElement("button");main.className="tsb-audio-main";main.type="button";main.setAttribute("aria-label","Play audio");main.innerHTML=playIcon;const volume=document.createElement("div");volume.className="tsb-audio-volume";const volumeMark=document.createElement("span");volumeMark.className="tsb-audio-volume-icon";volumeMark.setAttribute("aria-hidden","true");volumeMark.innerHTML=volumeIcon;volumeRange=document.createElement("input");volumeRange.className="tsb-audio-volume-range";volumeRange.type="range";volumeRange.min="0";volumeRange.max="100";volumeRange.step="1";volumeRange.setAttribute("aria-label","Volume");volume.append(volumeMark,volumeRange);player.append(audio,main,volume);document.body.append(player);main.addEventListener("click",()=>{if(audio.paused){userPaused=false;loadTrack(currentIndex,{time:audio.currentTime||0});}else{userPaused=true;audio.pause();saveState();setPlayerState();}});volumeRange.addEventListener("input",event=>setVolume(event.currentTarget.value));audio.addEventListener("play",()=>{blocked=false;setPlayerState();saveState();});audio.addEventListener("pause",()=>{setPlayerState();saveState();});audio.addEventListener("timeupdate",scheduleSave);audio.addEventListener("volumechange",()=>{updateVolumeUI();scheduleSave();});audio.addEventListener("ended",nextTrack);audio.addEventListener("error",()=>{setTimeout(nextTrack,700);});window.addEventListener("pagehide",saveState);document.addEventListener("pointerdown",()=>{if(blocked&&!userPaused)requestPlay();},{once:true,capture:true});const track=TRACKS[currentIndex];const source=track?pickSource(track):null;const resumeTime=source&&state.src===source.src?Number(state.time||0):0;updateVolumeUI();loadTrack(currentIndex,{time:resumeTime});}function initBookSearch(root=document){const input=root.querySelector("[data-book-search]");const grid=root.querySelector("[data-book-grid]");if(!input||!grid||input.dataset.searchReady)return;input.dataset.searchReady="1";const cards=[...grid.querySelectorAll(".book-card")];input.addEventListener("input",()=>{const q=input.value.trim().toLowerCase();cards.forEach(card=>{card.classList.toggle("hidden",q&&!card.dataset.title.includes(q));});});}function navigableUrl(anchor,event){if(event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return null;if(anchor.target&&anchor.target!=="_self")return null;if(anchor.hasAttribute("download"))return null;const href=anchor.getAttribute("href");if(!href||href.startsWith("#")||href.startsWith("mailto:")||href.startsWith("tel:"))return null;const url=new URL(href,location.href);if(url.origin!==location.origin)return null;if(url.hash&&url.pathname===location.pathname)return null;if(url.pathname.includes("/assets/")||url.pathname.includes("/audio/"))return null;const file=url.pathname.split("/").pop()||"";const ext=file.includes(".")?file.slice(file.lastIndexOf(".")).toLowerCase():"";if(ext&&ext!==".html")return null;return url;}function replaceFrom(nextDoc,selector){const current=document.querySelector(selector);const next=nextDoc.querySelector(selector);if(current&&next)current.replaceWith(document.importNode(next,true));}async function navigate(url,push=true){try{saveState();const response=await fetch(url.href,{credentials:"same-origin"});if(!response.ok)throw new Error("navigation failed");const html=await response.text();const nextDoc=new DOMParser().parseFromString(html,"text/html");if(!nextDoc.querySelector("main"))throw new Error("missing main");if(push)history.pushState({tsb:true},"",url.href);document.title=nextDoc.title;document.body.className=nextDoc.body.className;replaceFrom(nextDoc,"header.site-header");replaceFrom(nextDoc,"main");replaceFrom(nextDoc,"footer.site-footer");initBookSearch(document);window.scrollTo(0,0);}catch{location.href=url.href;}}function initNavigation(){document.addEventListener("click",event=>{const anchor=event.target.closest&&event.target.closest("a[href]");if(!anchor)return;const url=navigableUrl(anchor,event);if(!url)return;event.preventDefault();navigate(url,true);});window.addEventListener("popstate",()=>navigate(new URL(location.href),false));}function init(){initBookSearch(document);buildPlayer();initNavigation();}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init,{once:true});else init();})();"""
